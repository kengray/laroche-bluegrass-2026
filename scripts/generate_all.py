...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .expand-hint {{ font-size: 11px; color: var(--green-mid); margin-bottom: 10px; }}
...
function render(filterBy) {{
  ...
  days.forEach(day => {{
    ...
    dayBands.forEach(b => {{
      ...
      html += `<div class="band-card" onclick="this.classList.toggle('expanded')"><div class="card-header"><span class="band-name">${{b.name}}</span><span class="stage-pill ${{pillClass(b.stage)}}">${{b.stage}}</span></div><div class="card-meta">${{b.time}}${{b.country ? ' &middot; ' + b.country : ''}}</div><p class="expand-hint">Tap to expand</p>${{b.notes ? `<p class="card-notes">${{b.notes}}</p>` : ''}}<div class="card-links">${{links.join('')}}</div></div>`;
    }});
    html += '</div></div>';
  }});
