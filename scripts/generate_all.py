...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .hero-skyline {{ display: block; width: 280px; height: auto; margin: 0 auto 1rem; }}
  .hero-skyline polyline {{ fill: none; stroke: rgba(255,255,255,0.75); stroke-width: 2.5; stroke-linecap: round; stroke-linejoin: round; }}
  ...
  .hero-tagline {{ font-size: 11px; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; color: var(--gold-light); margin-bottom: 0.75rem; }}
...
<script data-goatcounter="https://laroche.goatcounter.com/count" async src="//gc.zgo.at/count.js"></script>
</head>
<body>
<header class="hero">
  ...
  <div class="hero-inner">
    <svg class="hero-skyline" viewBox="0 0 320 130" aria-hidden="true">
      <polyline points="0.0,101.2 2.2,101.2 4.4,101.2 6.6,101.2 8.8,101.2 11.0,101.2 13.2,101.2 15.4,103.4 17.6,105.6 19.8,107.8 22.0,110.0 24.2,112.2 26.4,114.4 28.6,116.6 30.8,116.6 33.0,116.6 35.2,116.6 37.4,116.6 39.6,116.6 41.8,114.4 44.0,94.6 46.2,92.4 48.4,90.2 50.6,88.0 52.8,88.0 55.0,88.0 57.2,90.2 59.4,90.2 61.6,90.2 63.8,90.2 66.0,91.7 68.2,93.1 70.4,94.6 72.6,94.6 74.8,94.6 77.0,94.6 79.2,94.6 81.4,94.6 83.6,94.6 85.8,94.6 88.0,96.8 90.2,90.2 92.4,90.2 94.6,88.0 96.8,83.6 99.0,79.2 101.2,81.4 103.4,85.8 105.6,88.0 107.8,90.2 110.0,95.2 112.2,100.1 114.4,105.0 116.6,110.0 118.8,110.0 121.0,101.2 123.2,99.0 125.4,97.9 127.6,96.8 129.8,77.0 132.0,77.0 134.2,74.8 136.4,74.8 138.6,72.6 140.8,70.4 143.0,70.4 145.2,68.2 147.4,66.0 149.6,66.0 151.8,66.0 154.0,66.0 156.2,66.0 158.4,68.2 160.6,94.6 162.8,96.8 165.0,99.0 167.2,99.0 169.4,99.0 171.6,99.0 173.8,99.0 176.0,100.1 178.2,101.2 180.4,103.4 182.6,105.6 184.8,107.8 187.0,107.8 189.2,107.8 191.4,108.9 193.6,110.0 195.8,112.2 198.0,114.4 200.2,114.4 202.4,114.4 204.6,114.4 206.8,114.4 209.0,114.4 211.2,114.4 213.4,114.4 215.6,114.4 217.8,116.6 220.0,118.8 222.2,121.0 224.4,121.0 226.6,121.0 228.8,121.0 231.0,121.0 233.2,121.0 235.4,121.0 237.6,121.0 239.8,121.0 242.0,121.0 244.2,121.0 246.4,99.0 248.6,96.8 250.8,94.6 253.0,92.4 255.2,68.2 257.4,66.0 259.6,59.4 261.8,39.6 264.0,22.0 266.2,39.6 268.4,59.4 270.6,66.0 272.8,90.2 275.0,92.4 277.2,94.6 279.4,96.8 281.6,99.0 283.8,100.1 286.0,101.2 288.2,101.2 290.4,101.2 292.6,101.2 294.8,101.2 297.0,101.2 299.2,101.2 301.4,101.2 303.6,101.2 305.8,101.2 308.0,101.2 310.2,101.2 312.4,101.2 314.6,101.2"/>
    </svg>
    <p class="hero-label">La Roche-sur-Foron, France</p>
    <h1>{FESTIVAL_NAME}</h1>
    <p class="hero-tagline"><span data-en>Europe&rsquo;s N&deg;1 Bluegrass Festival</span><span data-fr>Le n&deg;1 des festivals de bluegrass en Europe</span></p>
...
<main class="content">
  ...
  <div class="subscribe-section" id="subscribe-section" style="display:none;">
    ...
    <div class="platform-tabs cal-type-tabs" style="margin-bottom:1rem;">
      ...
      <button class="tab-btn" onclick="setCalType('camp',this)"><span data-en>Teaching camp only</span><span data-fr>Camp Bluegrass uniquement</span></button>
    ...
    <div class="cal-url" id="cal-url-display">https://kengray.github.io/laroche-bluegrass-2026/LaRoche2026-Festival.ics <button class="copy-btn" onclick="copyUrl()"><span data-en>Copy</span><span data-fr>Copier</span></button></div>
    ...
    <div id="tab-android" class="tab-content active steps">
      ...
      <ol>
        ...
        <li>Make sure <strong>Sync</strong> is ticked — it is off by default for newly added calendars.</li>
        <li>Go back to the main menu. The calendar will now be available for selection — check under <strong>Show more</strong> if it is not immediately visible. Tick the box next to the calendar name to enable it. The associated events will then appear.</li>
      </ol>
      ...
      <ol>
        ...
        <li>Assurez-vous que <strong>Synchronisation</strong> est activée — elle est désactivée par défaut pour les agendas nouvellement ajoutés.</li>
        <li>Revenez au menu principal. L'agenda sera désormais disponible — vérifiez sous <strong>Afficher plus</strong> s'il n'est pas immédiatement visible. Cochez la case à côté du nom de l'agenda pour l'activer. Les événements associés apparaîtront alors.</li>
      </ol>
...
<footer>
  ...
  <p style="margin-top:0.5rem;">&copy; 2026 Ken Gray &nbsp;&middot;&nbsp; Poster &copy; Paul Boutet / Roch&apos;&eacute;v&eacute;nements &nbsp;&middot;&nbsp; <span data-en>Built with</span><span data-fr>Réalisé avec</span> <a href="https://claude.ai" target="_blank">Claude</a> &nbsp;&middot;&nbsp; v{SITE_VERSION}</p>
</footer>
<script>
const BASE = 'https://kengray.github.io/laroche-bluegrass-2026/';
const CAL_URLS = {{
  en: {{
    festival: BASE + 'LaRoche2026-Festival.ics',
    camp:     BASE + 'LaRoche2026-Camp.ics',
    full:     BASE + 'LaRoche2026-Full.ics'
  }},
  fr: {{
    festival: BASE + 'LaRoche2026-Festival-FR.ics',
    camp:     BASE + 'LaRoche2026-Camp-FR.ics',
    full:     BASE + 'LaRoche2026-Full-FR.ics'
  }}
}};
let currentCalType = 'festival';
let currentLang = 'en';
function currentCalUrl() {{ return CAL_URLS[currentLang][currentCalType]; }}
...
function setCalType(type, btn) {{
  currentCalType = type;
  updateCalDisplay();
  document.querySelectorAll('.cal-type-tabs .tab-btn').forEach(b => b.classList.remove('active'));
