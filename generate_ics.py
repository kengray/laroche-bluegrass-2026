...
def make_event(uid, summary, date_str, start_time, end_time, description, location="La Roche-sur-Foron, France"):
    # date_str like "30/07/2026", times like "17:30", possibly "01:00" (next day)
    ...
    # If end time is earlier than start time, it's past midnight
    if end_dt <= start_dt:
...
main_rows = [
    # Thursday — start times from image; end = next act's start; Kristy Cox closes so end estimated
    ...
    # Friday — each act ends when next starts; Veranda closes so end estimated
    ...
    # Saturday — each act ends when next starts; Monogram closes so end estimated
    ...
    # Sunday — each act ends when next starts; Yellofox closes so end estimated
...
day_rows = [
    # Day stage — each act is exactly 1hr per the image slots; no overlaps
...
# ── TEACHING CAMP ─────────────────────────────────────────────────────────────
# Location: Lycée Sainte Famille, La Roche-sur-Foron
camp_rows = [
    # Monday 27 July
    ...
    ["Tue 28 Jul", "Today's news + Instrumental classes",   "Teaching Camp", "09:00", "10:30", "Morning session 1 — instrumental classes"],
    ...
    ["Tue 28 Jul", "Wernick Method Jam: Instructions",      "Teaching Camp", "14:00", "16:00", "Afternoon — Wernick Method jam instructions"],
    ["Tue 28 Jul", "Break",                                 "Teaching Camp", "16:00", "16:30", ""],
    ["Tue 28 Jul", "Optional/requested modules",                  "Teaching Camp", "16:30", "19:00", "Requested/optional modules"],
    ...
    ["Wed 29 Jul", "Wernick Method Jam: Coached jam",       "Teaching Camp", "14:00", "16:00", "Afternoon — coached Wernick Method jam"],
    ["Wed 29 Jul", "Break",                                 "Teaching Camp", "16:00", "16:30", ""],
    ["Wed 29 Jul", "Concert rehearsal & organisation",               "Teaching Camp", "16:30", "18:00", "Concert organisation / rehearsal for Thursday student concert"],
    ...
    ["Thu 30 Jul", "Band labs / jams",                      "Teaching Camp", "11:00", "12:30", "Final morning session — band labs and jams"],
    ...
    ["Thu 30 Jul", "Head to festival site",                 "Teaching Camp", "16:30", "17:30", "Move to Lycée Sainte Marie festival site"],
    ["Thu 30 Jul", "Students concert — Main Stage",         "Teaching Camp", "17:30", "19:00", "Band Labs perform on the Main Stage"],
    ["Thu 30 Jul", "French teachers set — Main Stage",      "Teaching Camp", "19:00", "20:00", "French teachers perform on the Main Stage"],
]
...
for row in main_rows + day_rows + street_rows + camp_rows:
    date_key = row[0]
    name, stage, start, end, desc = row[1], row[2], row[3], row[4], row[5]
    loc = "Lycée Sainte Famille, La Roche-sur-Foron, France" if stage == "Teaching Camp" else "La Roche-sur-Foron, France"
    emoji = "🎓" if stage == "Teaching Camp" else "🎸"
    summary = f"{emoji} {name} — {stage} | Bluegrass La Roche 2026"
    events.append(make_event(f"blr2026-{uid_counter:03d}", summary, date_map[date_key], start, end, desc, location=loc))
    uid_counter += 1
